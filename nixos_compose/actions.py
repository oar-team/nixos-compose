...
def generate_deployment_vm(compose_info, ssh_pub_key_file=None):
    ...
    json_deployment = json.dumps(deployment, indent=4)
...
def wait_ssh_ports(ctx, ips, halo=True):
    ...
    nb_ips = len(ips)
    nb_ssh_port = 0
    waiting_ssh_ports_cmd = f"nmap -p22 -Pn {' '.join(ips)} -oG - | grep '22/open' | wc -l"
...
def launch_driver_vm(ctx, deployment, flavour, ips, httpd_port=0, driver_repl=False, test_script=None):
